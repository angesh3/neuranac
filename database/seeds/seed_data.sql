...
-- Demo-tools container (common Docker Compose network IPs for radtest)
INSERT INTO network_devices (tenant_id, name, ip_address, device_type, vendor, model, shared_secret_encrypted, status)
SELECT t.id, 'demo-tools', '172.18.0.2', 'virtual', 'Test', 'radtest', 'testing123', 'active'
FROM tenants t WHERE t.slug = 'default'
ON CONFLICT (tenant_id, ip_address) DO NOTHING;
INSERT INTO network_devices (tenant_id, name, ip_address, device_type, vendor, model, shared_secret_encrypted, status)
SELECT t.id, 'demo-tools-alt', '172.19.0.2', 'virtual', 'Test', 'radtest', 'testing123', 'active'
FROM tenants t WHERE t.slug = 'default'
ON CONFLICT (tenant_id, ip_address) DO NOTHING;
